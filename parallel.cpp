...
// Data structure for traffic data
struct traffic_data
{
    int id;
    std::string times;
...
// Comparison function for sorting traffic data based on the number of cars
...
void *producer(void *args)
{
    while (ProducerCount < m)
    {
        ...
        lock.unlock();
        sleep(rand() % 5); // Introduce random sleep time
    }
}
    ...
    while (consumerCount < m)
    {
        ...
        if (!signal_queue.empty())
        {
            ...
            if (sig.traffic_light_id >= 1 && sig.traffic_light_id <= 10)
            {
                ...
                sorting_array[sig.traffic_light_id - 1].numofcars += sig.numofcars; // Adjust index to start from 0
            }
        ...
        if (consumerCount % (Hours * PThreads) == 0 && consumerCount != 0)
        {
            ...
            sort(sorting_array, sorting_array + 10, sort_method);

            // Print top 5 entries
            cout << "Highest 5 Entries:" << endl;
            for (int i = 0; i < min(5, 10); i++)
            {
                // Display traffic_light_id starting from 1
                cout << "Light ID: " << sorting_array[i].traffic_light_id << " , Number of Cars: " << sorting_array[i].numofcars << endl;
            }
...
void get_traff_data()
{
    ...
    for (const auto &ts : unique_timestamps)
    {
        ...
        {
            cout << "Light ID: "<< filtered_data[i].traffic_light_id <<" , Number of Cars: " << filtered_data[i].numofcars << endl;
        }
        cout << endl;
    }
}
